...
export type OpaqueAttrs = {
   ...
   * apart unnoticed.
   *
   * A hole is an output that takes the promotion for a hand-authored DSL
   * barrier. Imported barriers may not use this convenience: preflight requires
   * one concrete dtype per output because an unknown source operation's result
   * type cannot be inferred safely from its inputs.
   */
  dtypes?: (DType | null)[];
  /**
