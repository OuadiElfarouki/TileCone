...
The canvas assigns one perceptual channel to each fact. Hue identifies the selected tile. Uniform fill means **What it needs**; a `\` ruling at a CSS-pixel pitch means **What it feeds**; a dot stipple means **What it is combined with**; the opposite dashed `/` hatch means a conservative over-approximation.
