...
The current reuse estimator is UI-local in `Inspector.tsx`. It samples dependency propagation rather than belonging to the deterministic core metrics API; this is an explicit architectural boundary rather than part of `computeMetrics`.
...
- `App.tsx` composes the identity header, source panel, graph, and tile inspector. It also restores compatible URL state and falls back to a built-in example.
...
- `GraphView.tsx` uses Dagre for collision-free base positions, applies persistent user tensor offsets, routes live cubic connectors, and manages pan/zoom/focus. Drag motion is constrained against the full rectangles of every tensor and operator.
...
- `share.ts` holds link encoding and decoding together. They were split across a toolbar and `App` once, and when the toolbar was removed the encoder went with it, leaving the app able to restore links nothing could produce. A malformed payload is refused rather than repaired: a half-restored workspace is worse than the default example.
