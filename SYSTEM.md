...
Overlap is therefore resolved where quantities are measured, not where they are produced. `disjointify` is that form - split-on-overlap, then merge - and it is a partition, so volumes may be summed over it. The rule every consumer must know is one sentence:
...
Four consumers measure and therefore disjointify: `count` (and so `elements`, `bytes`, and the percentage), `points`, the per-box FLOP sum in `metrics.ts`, and the canvas **fill** in `grid.ts`. The last is not pedantry - each rect is composited at the layer's alpha, so a shared element painted twice would read darker than its neighbours and make alpha mean multiplicity by accident, on the one channel the renderer deliberately holds constant.
...
`coversAxisFully` survives this change and still proves containment rather than trusting a spanning box. The `matmul(A, A)` case that originally motivated it now passes the naive test, since the row band is one box again - but that is a case going right, not a guarantee. Two boxes can still cover an axis between them with neither spanning it, and only boxes agreeing on every axis but one are merged.
...
Fit may go below the manual legibility floor to include the whole scene. Its 20px margin is measured on screen so overview labels at the top remain inside the viewport. Zooming in from that overview is gradual; manual zoom-out cannot shrink it further. Hover coordinate tooltips counter-scale to their normal screen size, and `⊞` distinguishes the lattice span from tensor shape.
...
`src/ui/grid.ts` is a pure geometry and drawing layer. Regions are painted at **element** precision: `regionRects` maps each box straight to canvas pixels, so a region that ends mid-tile draws a crisp edge rather than a partially shaded cell. The tile lattice is drawn over the top as a reading aid and does not quantise the marks. A thin region is widened to a one-screen-pixel minimum (limited by the whole canvas), expanding inward at the far edge to retain the floor, which over-states extent rather than letting it disappear - the same conservative direction the region algebra takes. Perimeter emphasis is suppressed when its stroke would be wider than the mark, so a one-element point or line never grows false area. The lattice threshold and stroke widths are measured in screen pixels. Canvas backing resolution follows DPR and coarse zoom buckets, and unchanged backing stores are reused; CSS uses ordinary interpolation rather than re-pixelating that supersampled result.
