...
Fit may go below the manual legibility floor to include the whole scene. Its 20px margin is measured on screen so overview labels at the top remain inside the viewport. Zooming in from that overview is gradual, and zooming back out returns to it: `lowZoomBound` takes the lower of the legibility floor and the current scene's fitted scale, and takes no current scale as an argument. Deriving the clamp from the gesture instead let zooming in past the floor raise the floor, which left the fitted view reachable only through `fit`. Manual zoom-out stops at the fitted scale and no further. Hover coordinate tooltips counter-scale to their normal screen size, and `⊞` distinguishes the lattice span from tensor shape.
