...
│   ├── analysis.worker.ts  off-thread compiler, layout, and family-analysis host
...
│   ├── analysis-worker-client.ts  request correlation and Worker lifecycle
...
- `App.tsx` composes the identity header, source panel, graph, and tile inspector. URL state crosses one asynchronous transactional store boundary: the DSL is compiled in the analysis Worker and every selected part is validated before any source, graph, settings, or selection state is committed. Failure falls back to a built-in example without exposing a half-restored graph.
...
- `PanelFrame.tsx` owns side-panel width, the mouse- and keyboard-adjustable drag strip, and collapse-to-rail for both panels. Collapsing unmounts transient panel UI while store-owned drafts and analysis state survive; focus moves to the replacement rail or collapse control instead of falling onto the document.
...
- `graph-scene.ts` owns the two-stage graph rendering pipeline. In the browser's first stage, the analysis Worker combines a resolved graph with the pure card-measurement policy and produces the immutable Dagre base layout. The main realm combines that base with persistent tensor offsets to produce the cheap live scene and routed connectors. Query results, highlighting, and viewport state cannot invalidate structural placement. Links are rebuilt from ordered operation operands rather than Dagre's endpoint-keyed edge set, so repeated operands such as `matmul(X, X)` remain distinct.
...
2. The analysis Worker parses the text, creates source mappings, resolves the graph, and runs Dagre.
...
The core jobs remain deterministic and usable synchronously in headless code. Browser compilation, structural layout, and whole-family analysis are asynchronous so they cannot monopolize the UI thread. React components do not implement operation semantics; they only create queries and render results.
