...
│   └── plan/
│       ├── tile-family.ts  regular tilings: tails, lazy coordinates, the tiles a box meets
│       ├── plan.ts         tile families checked against a graph, and each task's demand
│       └── interfaces.ts   producer/consumer joins and demand across a whole family
├── parse/
...
`src/core/region.ts` owns region algebra and normalization. A region's boxes **may overlap**. `canonicalize` normalizes only in union-preserving ways - it drops empty boxes, drops boxes contained in another, and merges boxes that agree on every axis but one - and runs those to a fixpoint. It never splits a box on overlap. If the box count exceeds the configured cap it coarsens toward the cap by merging neighbours into their hulls, and records the loss of precision.

Both simplifications are ordered rather than searched, because the box counts that reach them are ordinary. A reshape decomposes one tile into up to `reshapeRuns` contiguous runs and a strided operation enumerates up to `stridedEnum` positions, all before the cap applies, so a few thousand boxes arrive from a three-line graph. Merging groups the boxes by their other axes and sweeps each group in order, and containment sorts by lower corner ascending and upper corner descending, which puts a container before anything it contains and makes the result independent of the order the boxes arrived in. Both previously compared every pair, and merging restarted its scan from the first box after each success, so a tile drawn across a flattening reshape spent about 300ms per query. Containment carries a work budget: it is a simplification, so stopping early leaves boxes that could have been dropped and changes nothing else.
...
Difference and intersection both multiply boxes - every subtrahend box can cut every fragment into up to `2 * rank` pieces, and two regions of N boxes can meet in N² - and neither is allowed to shorten its own work by handing back a bound. The box cap is the only approximation in this module, and it applies to the set those operations actually computed rather than standing in for computing it. A crossing family of 256 stripes a side therefore still builds all 65536 pieces before the cap coarsens them, which is what keeps the answer the same set it always was.
...
`crossings` splits that demand by operand slot. It has one entry per node and slot through which the cone reaches a frontier tensor, with the region carried through that slot. A tensor read through two slots, as in `matmul(X, X)`, has two entries, and their union is the tensor's region in `tensors`. The walk computes each slot's contribution before merging it into the tensor's region, and a bounded walk keeps that contribution for tensors on the frontier.
...
`supplyOf(plan, task)` in `interfaces.ts` joins each slot's demand with the family of the tensor it reads. A producer task is needed when the demand meets its tile. The intersection is the witness, recorded per consumer slot. A tile met through two slots is listed once with two witnesses, so a repeated operand does not repeat a dependency. Each demand is classified by its supplier:

- `input`: a graph input, read from memory and produced by no task. A graph input read by two operations gives neither a producer.
- `tasks`: a planned tensor, supplied by its family's tasks.
- `unplanned`: a produced tensor the plan does not tile. Its producer tasks are not specified, and the supply's `complete` flag is false.
...
Each task costs one bounded query plus one intersection per producer tile it meets. A family with more tasks than the budget, 4096 by default, is declined with status `over-budget` and no figures. A sum over part of a family is a lower bound on its total, and no figure in the app is reported as a lower bound. In the browser, the family sweep runs in the query Worker; only the finished report crosses back to React.

Plan and task errors are `PlanError`s with stable codes: `PLAN_UNKNOWN_TENSOR`, `PLAN_GRAPH_INPUT`, `PLAN_TILE`, `PLAN_SIZE`, `PLAN_UNPLANNED` and `PLAN_TASK`.
...
Actions defined against one tensor's axes - arrow-key moves, hidden-axis sliders, the reuse estimate, the axis legend - follow the active analysis group. A focused tile narrows the action within that group; when an action requires one concrete tile and none is focused, the most recently drawn enabled tile in the group is the anchor. Those that *edit* the selection - the arrow moves, the removal key, and the include/exclude key - additionally require the inspector to be on Dependencies, the view whose list shows which tile they act on. Removal asks for more than the others: it acts only on a *focused* tile, hovered or pinned, never on the anchor fallback, because Escape drops the focus precisely to say the reader is done pointing at it and a destructive key must not outlive that; view toggles stay global, because the canvas shows what they did. Dependencies and Execution are an ordinary two-button group, not ARIA tabs: both buttons are tab stops, Enter and Space activate them, and neither claims the arrow navigation that a tab pattern would require. That choice preserves arrow keys for moving the tile even after the view switch has been clicked. An axis index means different things on tensors of different rank, so there is no single axis such an action could apply everywhere.

Expanding a composite node first resolves shape context, so composites fed by inferred intermediate tensors have their true rank available. The rewrite is serialized back to primitive DSL and compiled through the same Worker boundary, keeping the displayed source, share links, unresolved graph, and resolved graph synchronized. Selection state that could no longer be valid is cleared. Its undo entry retains the previous resolved graph and base layout, so undo restores them directly instead of recompiling and laying out on the UI thread.

### Main components

Execution scope is reapplied transactionally after selection edits and undo,
including a first draw in an empty Execution view. Above the twelve-part
attribution cap, one dedicated anchor query supplies the scoped figures and
cones; leaving Execution restores the grouped analysis. Playback-only state
updates do not rerun these queries. Display subtraction is cached by geometry
and limited to 2,048 box pairs and 256 fragments; exceeding either budget keeps
the full reach visible with an approximation mark. Estimator figures are
unaffected by this rendering fallback.

- `App.tsx` composes the identity header, source panel, graph, and tile inspector. URL state crosses one asynchronous transactional store boundary: the DSL is compiled in the compile Worker and every selected part is validated before any source, graph, settings, or selection state is committed. Failure falls back to a built-in example without exposing a half-restored graph.
- `WorkspaceHeader.tsx` contains product identity, description, the shortcut-sheet entry point, and the global theme control.
- `SidePanel.tsx` owns the editable DSL draft, share action, built-in examples, and operation list. The last successfully built source and the current editor draft are separate store fields: choosing an example or typing keeps the built graph and analysis live, while the operation list is labelled and dimmed as belonging to the built graph. A synchronized, pointer-transparent syntax layer colors comments and stable DSL keywords beneath the native textarea. Running the draft starts the Worker job, exposes a building status, and replaces the workspace only after compilation and layout succeed. Build status is the textarea's accessible description, and a failed build marks the field invalid and announces the collected diagnostics.
- `GraphView.tsx` is the graph controller and React projection: it consumes the Worker's base layout, derives highlighting, manages pan/zoom/focus and tensor gestures, and renders the current scene. A synchronous layout fallback exists only for headless callers that installed a graph without a Worker artifact. Viewport panning starts on every background surface, including the transformed inner layout behind connectors; tensor cards, operation nodes, and both HUD clusters retain their own gestures. The card header is the full-width drag surface and the dotted button remains its visible and keyboard-focusable affordance; the tensor name is carved back out of it, because it is the focus target for the shape popover and a drag there would swallow the click that opens it. The world is unbounded, scene bounds include negative card positions, useful zoom is bounded by tensor legibility and canvas supersampling, and reset layout is an undoable recovery action. Connector SVGs are split by meaning: dim context runs behind the plates, while ordinary and hot connectors cross above them, including during a card drag. It does not own layout or routing algorithms.
...
- `TensorCard.tsx` renders an interactive tensor grid on canvas and converts pointer gestures into element-space boxes. Its paint stack is built by pure helpers: `buildLayers` owns cone encoding, `buildPlanPaint` owns declared tasks, and `buildExecutionPaint` overlays the reuse probes the estimator actually performed. During playback each sampled tile pulses once on the studied tensor, and everywhere else the probe pulses as whatever it reaches through each traced relation, in that relation's own mark - a solid fill for what it needs, a ruling for what it feeds, a stipple for what it is combined with - with the part the anchor also reaches drawn at full strength over the rest. Borrowing the marks rather than inventing new ones is what keeps a probe's upstream reach from reading as its downstream reach. Completed probes settle into a quieter summary of both readings, whose approximation carries through from any probe that was widened. The overlay draws no lattice of its own. The sweep's cover - tiles of the anchor's extents laid over the tensor - would read naturally as one, but a card still takes ordinary selection drags under Execution and those snap to the square display tile, which the anchor's extents generally are not, so a drawn cover would put lines on the canvas that no gesture lands on (§9). The Plan view can draw its lattice because a gesture on a tiled card inspects rather than draws. The probe rectangles carry the cover instead, and carry only the tiles the sweep actually walked rather than every tile a full lattice would imply. The overlay never replaces `selection` or `plan`, and its opacity falls over 240 ms when another view is chosen, so the next view is already present underneath rather than appearing after a hard reset; returning to Execution during that fade settles the sweep rather than letting it finish leaving. Hover probes compute both directions under the `MAX_PREVIEW_NODES` cap; the view filter paints solid needs, ruled feeds, both, or neither without re-querying. Hover previews are selected per tensor, repeated pointer frames inside one logical cell are ignored, and what survives that is coalesced to one query per animation frame by `useFrameThrottle` - pointer events outrun frames by an order of magnitude, and only the last position in a frame can be seen. Every preview update goes through the throttle, clears included, so a clear cannot be overtaken by a move still pending for the frame; the canvas effect declares its paint dependencies, so a hover does not re-rasterise unrelated cards.
- `Inspector.tsx` is ordered by the question the product asks: the tile's identity, the tiles list, then **Backward Cone** and **Forward Cone**, followed by approximation, cost, sharing, and dependency notes. Each directional heading is also its adjacent view toggle and its fill chip is the legend for the matching canvas treatment; each tensor appears once per direction with its box count, segmented per-tile footprint, slice expressions, and copy action. The Forward Cone rollup carries completed-versus-partial counts so the product's distinguishing verdict is visible before rows are scanned. Dependency severity uses progressively heavier rules in muted, warning, and error ink; accent remains reserved for active UI. The panel is read-only apart from the tile list: the lattice, snap and axis reading changed only the canvas, so they now ride the canvas HUD and the panel keeps nothing but the readout and the tiles it is scoped to. With nothing drawn the panel names the three relations it will answer and offers a starting tile rather than rendering null sections.

- The panel is split into two views, and the split is by what a figure rests on rather than by subject. **Dependencies** holds every figure that is a function of the graph and the drawn region: the cones, the co-access surface, footprints, FLOPs and bytes, duplicate graph-input demand across the drawn tiles, and the notes. Each is exact or carries the kind of bound its structure supports. **Execution** holds the figures that exist only once an execution is assumed - a tiling of the whole tensor, an order, a fusion decision - which is where the fused and unfused arithmetic intensities and the sampled reuse sweep live, under a note saying so. They were separated because they fail differently: a dependence figure fails by being conservative, a modelled one by being wrong in either direction, and a model rendered beside a bound borrows its credibility. The choice is view state on the store rather than local, so it survives a redraw, and the tile header sits above the switch because both classes describe the same tile. What splits along this line is the question, not the subject: 'what duplicate demand do these drawn tiles create' is unsampled and stays under Dependencies, while 'what would every tile of this size share' assumes a tiling nobody drew and moves. A completed sweep is local state keyed by both the resolved graph and its exact probe box; changing either hides it synchronously, before an old tensor ID can be read against a new graph. Replaying does not move the real selection, but opening Execution does narrow it: a sweep is defined by one tile and the figures beside it are about that tile, so every other tile is disabled on the way in and put back on the way out. Disabling is the existing control rather than a second kind of invisibility - each tile keeps its faint rectangle, so the reader can see they are still there. The tile swept is the focused one when it is in the group and enabled, else the last enabled one drawn there, and `sweepAnchorIndex` is shared with the inspector's own probe so the view cannot scope to one tile and sweep another. What is restored is what was set aside rather than what Execution ended up holding, and it is held by part identity, the way `setSelection` remaps it, so an edit that renumbers the tiles cannot put the wrong ones back; a part that no longer exists drops out. Returning to Dependencies therefore reveals the original drawn tiles; moving directly to Plan preserves an existing inspected task, or creates task coordinate zero on the studied produced tensor with the sweep's tile extents when no task exists.
- `shortcuts.ts` is the single manifest for key matching and help copy. `useKeyboard.ts` dispatches global bindings, local owners such as the source editor match against the same manifest, and `ShortcutsDialog.tsx` renders its grouped inventory. The shortcut sheet traps focus, makes the workspace inert, restores the control that opened it, and suspends every global binding except Escape while open.
...
- `PlanPanel.tsx` is the Plan view (§8b): the tiled tensors and their extents, the inspected task with what it reads and which producer tasks supply it, and the same questions asked of its whole family. It is a third inspector view because a plan figure rests on a declared tiling rather than on the drawn region, which is neither promise the other two make. It is reachable with nothing drawn, since a task is not a drawn tile. Reads are grouped by tensor rather than by operand slot, because the producers, the supplier and the action that tiles a tensor belong to the tensor: per slot, an operation reading one tensor twice printed its producer list twice. Long producer lists are cut to the first few behind a control, with the full count stated above them. Escape releases the inspected task and leaves every tiling, and an undo that restores a plan edit brings this view forward so the step back is visible. A family of at most 256 tasks is evaluated as it is shown and a larger one on request; either path runs in the query Worker and stale reports are ignored. The dependency matrix is drawn only while both families are at most 64 tiles.
- In the Plan view a card paints the plan rather than the cones, through `buildPlanPaint`. The inspected task's tile is drawn like a placed tile, what it reads is the solid needs fill in the same hue with the approximation hatch when widened, and the lattice is the plan's tiling instead of the display one. A tensor the plan does not tile has no lattice, which is how a graph input and an untiled intermediate read as having no tasks. The producer tiles the task needs are outlined in neutral ink, dashed where the dependency is only possible, so the unfilled part of an outlined tile is the part the consumer does not read. A tensor is divided once, by drawing: a drag over an untiled tensor covers it at the extents drawn, and every gesture after that inspects the task under the press, since redrawing would move the lattice under a reader mid-study and change what the figures beside it are about. Dividing it differently means clearing the tiling or typing extents. `planGesture` decides which of the two a gesture is. A plan drag always snaps to the drawn lattice whatever the snap toggle says, and the toggle is disabled in this view: a plan extent is a quantity, and one taken from wherever a pointer stopped is not a tiling anyone chose. The arrow keys step the task by a tile, as they move a drawn tile under Dependencies. An untiled produced tensor is divided at the tile the canvas is drawing on its visible axes and one element on the others, and the produced tensors its operation reads are divided with it, so a first click names producer tasks rather than reporting that the tensor holding them is untiled. Until a tensor is tiled its card carries that proposed cover faintly, which is how the view says which tensors have tasks at all. The canvas grid seeds those extents and never steers them again: a plan that followed the view would mean something else at another zoom.
- `PanelFrame.tsx` owns side-panel width, the mouse- and keyboard-adjustable drag strip, and collapse-to-rail for both panels. Collapsing unmounts transient panel UI while store-owned drafts and analysis state survive; focus moves to the replacement rail or collapse control instead of falling onto the document. The execution playback is the one piece of store state that does not survive it: the timers that advance and retire the overlay live in the inspector, so leaving it behind would freeze the probes on every card with nothing able to clear them.
...
- `graph-scene.ts` owns the two-stage graph rendering pipeline. In the browser's first stage, the compile Worker combines a resolved graph with the pure card-measurement policy and produces the immutable Dagre base layout. The main realm combines that base with persistent tensor offsets to produce the cheap live scene and routed connectors. Query results, highlighting, and viewport state cannot invalidate structural placement. Links are rebuilt from ordered operation operands rather than Dagre's endpoint-keyed edge set, so repeated operands such as `matmul(X, X)` remain distinct.
...
2. The compile Worker parses the text, creates source mappings, resolves the graph, and runs Dagre without waiting behind family or reuse work.
3. The store accepts the response only if it is still current, then atomically replaces the graph and base layout or retains compilation diagnostics for display.
4. Tensor cards derive their grids from resolved shapes while `GraphView` projects the returned base layout through the current user offsets.
...
The core jobs remain deterministic and usable synchronously in headless code. Browser compilation and structural layout use one Worker lane; whole-family and reuse analysis share a separately cancellable query lane. They cannot monopolize the UI thread or block a build behind stale analysis. React components do not implement operation semantics; they only create queries and render results.
...
- the Plan view: that a click tiles a produced tensor and names the task under the pointer, that a graph input is ignored, that retiling keeps the task on the same elements, that plan edits undo like tile edits, that a graph replacement clears the plan, and that the paint draws the task as a placed tile, the demand as one fill however many slots read it, and a lattice only where the plan tiles;
- tile families: every element in exactly one tile across random shapes and extents, including shortened tails and extents wider than the axis, and the tiles a box meets agree with a scan of the whole family;
- tile plans against a task oracle built from `oracleDeps` alone: per-slot demand, the producer set, and each family's summed and distinct bytes, readers and fan-out, on random graphs with random tilings. Under lowered fallback thresholds the same check requires that no true producer is missed and that a producer reached only through widened demand is not called definite. That corpus asserts it produced such producers, including producers the truth does not have;
- the two-matmul chain as fixed cases: producer counts, row-band sharing, the effect of a taller consumer tile, partial use when tilings do not align, and duplication factors on both inputs;
...
- A bounded cone has no metrics, notes, or contribution report; those are defined over transitive cones. Tile plans report demand, duplication and fan-out only. Staged reads, partial work and live storage are not modelled.
- Plan tasks are single operations that compute their whole reduction. Fused multi-operation tasks, split reductions, worker placement and execution order do not exist yet.
- A plan is workspace state, not part of a share link or the DSL. A shared link carries the source and the selection, and the recipient opens it with nothing tiled.
