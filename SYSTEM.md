...
Reuse is a separate derived analysis in `src/core/reuse.ts`, rather than part of `computeMetrics`, and it answers two questions that share the word. `inputSharing` is the exact one: over the enabled tiles' backward cones it compares the bytes each input would be read independently against the bytes of their union, so the difference is traffic a kernel could actually avoid rather than a sample. It reports no duplicate figure at all when a region was widened, because the difference of two bounds is not itself a bound. `estimateInputReuse` is the sampled one: it takes one anchored selection, walks same-sized tiles across that tensor with a fixed seed, and uses checked backward queries to count how many touch each input region in the anchor's footprint. One sample is drawn per stratum and carries that stratum's weight, so an uneven grid - five tiles under two probes - is not biased by the hit rate alone, and an exhaustive sweep returns the count itself rather than an estimate of it. Only a probe that reports a touch can have reported it falsely: regions are over-approximations, so an empty intersection of two of them is an empty intersection of the truth. That keeps the count an upper bound rather than an unknown, and keeps a distant approximation the estimate never relied on from marking it inexact. The sweep also probes the immediate neighbours - one tile step along each axis, with out-of-bounds steps skipped rather than resized - and those carry their own precision, because they inform the reader rather than the count. Keeping all of it pure and seeded makes repeated estimates reproducible without conflating a sampled statistic with the exact cost metrics; the inspector keeps that distinction visible by rendering the exact half under Dependencies and the sweep under Execution.
...
Actions defined against one tensor's axes - arrow-key moves, hidden-axis sliders, the reuse estimate, the axis legend - follow the **anchor**: the focused part's tensor, else the last one drawn on. Those that *edit* the selection - the arrow moves, the removal key, and the include/exclude key - additionally require the inspector to be on Dependencies, the tab whose list shows which tile they act on; view toggles stay global, because the canvas shows what they did. The tab strip itself answers no arrow key and keeps both tabs in the tab order rather than taking the usual roving tabindex: the arrows already belong to the tile, and a focused tab that consumed them shadowed that binding the moment the tab was clicked. An axis index means different things on tensors of different rank, so there is no single axis such an action could apply everywhere.
...
- The panel is split into two tabs, and the split is by what a figure rests on rather than by subject. **Dependencies** holds every figure that is a function of the graph and the drawn region: the cones, the co-access surface, footprints, FLOPs and bytes, what the drawn tiles read in common, and the notes. Each is exact, or a bound that names why it was widened. **Execution** holds the figures that exist only once an execution is assumed - a tiling of the whole tensor, an order, a fusion decision - which is where the fused and unfused arithmetic intensities and the sampled reuse sweep live, under a note saying so. They were separated because they fail differently: a dependence figure fails by being conservative, a modelled one by being wrong in either direction, and a model rendered beside a bound borrows its credibility. The tab is view state on the store rather than local, so the choice survives a redraw, and the tile header sits above the strip because both classes describe the same tile. What splits along this line is the question, not the subject: 'what do these tiles share' is exact and stays under Dependencies, while 'what would every tile of this size share' assumes a tiling nobody drew and moves.
